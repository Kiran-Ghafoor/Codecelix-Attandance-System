import { Mail, Phone, Layers, CalendarDays, UserCog } from "lucide-react";
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <Card>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-steel-500">{user?.batch}</p>
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <Field icon={Layers} label="Batch" value={user?.batch} />
          <Field icon={CalendarDays} label="Joined" value={user?.joinDate} />
          <Field icon={UserCog} label="Mentor" value={user?.mentor} />
        </div>
      </Card>
    </div>
  );
