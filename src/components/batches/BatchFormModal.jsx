...
import { buildBatchFormSnapshot, comparableDomains, isBatchFormSaveable } from "../../lib/batchForm";
...
// Create/edit form for batches. Supports multiple domains, each with a name
// and an optional team leader picked from the user list. `initial` is an
// existing batch (edit mode) or null (create mode).
export default function BatchFormModal({ open, onClose, title, initial = null, onSubmit }) {
  // The form is keyed by batch id so it remounts fresh on every open/edit
  // target change instead of syncing state through an effect.
  return (
    <Modal open={open} onClose={onClose} title={title} size="xl">
      {open && <BatchForm key={initial?.id ?? "new"} initial={initial} onClose={onClose} onSubmit={onSubmit} />}
    </Modal>
  );
}

function BatchForm({ initial, onClose, onSubmit }) {
  ...
  // Inline messages for date fields, shown as soon as an entry is invalid.
  const [dateErrors, setDateErrors] = useState({});
  // Captured once on mount; the modal remounts the form per open/edit target.
  const snapshot = useMemo(() => buildBatchFormSnapshot(initial, rows), []); // eslint-disable-line react-hooks/exhaustive-deps

  // Team leaders can only be internees enrolled in THIS batch. A brand-new
  // batch has no members yet, so leader picking unlocks after it exists.
  ...
  const form = { name, startDate, endDate, status, rows };
  const valid = isBatchFormSaveable(form, snapshot);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <p className="text-[12px] text-steel-400">This is a UI mock — changes aren't persisted yet.</p>
    </form>
  );
}
